...
fn detect_single_fast(path: &Path, page_rotation: Rotation) -> Option<(CropBox, (u32, u32))> {
    ...
    imgproc::threshold(&blurred, &mut thresh, 0.0, 255.0, imgproc::THRESH_BINARY | imgproc::THRESH_OTSU).ok()?;
    ...
    for pt in &[
        core::Point::new(0, 0), core::Point::new(d_cols - 1, 0),
        core::Point::new(0, d_rows - 1), core::Point::new(d_cols - 1, d_rows - 1)
    ] {
        if let Ok(val) = thresh.at_2d::<u8>(pt.y, pt.x) {
            if *val == 255 { corner_white_count += 1; }
        }
    }
    ...
    imgproc::find_contours(&closed, &mut contours, imgproc::RETR_LIST, imgproc::CHAIN_APPROX_SIMPLE, core::Point::new(0, 0)).ok()?;
    ...
    for i in 0..contours.len() {
        ...
        if rectangularity > 0.45 {
            ...
            let sliver_penalty = if width_ratio < 0.15 || height_ratio < 0.15 { 0.15 } else { 1.0 };
            ...
            let score = (area / total_area) * rectangularity.powi(2) * centrality_score * aspect_score * sliver_penalty * brightness_score;
    ...
    let crop = best_candidate.map(|(_, rect)| {
    ...
    }).unwrap_or(CropBox { x: 0, y: 0, w: final_w, h: final_h });
...
pub fn detect_all_pages(paths: &[(usize, &Path, Rotation)]) -> Vec<(usize, CropBox, u32, u32)> {
    ...
    paths.par_iter().filter_map(|&(idx, path, rotation)| {
        let (crop, (w, h)) = detect_single_fast(path, rotation).unwrap_or_else(|| {
            // Fallback: Return the full image size if detection fails.
            let src = imgcodecs::imread(path.to_str().unwrap_or(""), imgcodecs::IMREAD_UNCHANGED).ok();
            if let Some(s) = src {
...
pub fn cluster_by_similarity(crops: &[(usize, CropBox)]) -> Vec<CropCluster> {
    ...
    for i in 0..crops.len() {
        ...
        for j in (i + 1)..crops.len() {
            ...
            let area_sim = if area_i > area_j { area_j / area_i } else { area_i / area_j };
